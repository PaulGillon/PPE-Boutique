...
class client {
	...
	// Test pour savoir si l'utilisateur qui se connecte est admin ou pas

	public function testAdmin($email)
	{
		$sql = "SELECT COUNT(*) FROM client WHERE emailClient = :mail AND admin = 1";
		...
		if($ligne[0] == 1)
		{
			$_SESSION["admin"] = $ligne[0];
			return true;
			
		}
		else
		{
			$_SESSION["client"] = $ligne[0];
			return false;	
		}
