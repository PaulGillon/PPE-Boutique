...
class categorie {
	...
	public function getImagesCategorie($categorie)
	{
		$sql = "SELECT idProduit,imgProd FROM photos INNER JOIN produit ON produit.codeProduit = photos.idProduit WHERE idCategorie = :idCat";
		$req = $this->pdo->prepare($sql);
		$req->bindParam(':idCat', $categorie, PDO::PARAM_INT);
		$req->execute();
